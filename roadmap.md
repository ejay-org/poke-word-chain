...
- [x] **Step 8**: 단어 검증 로직(`validateWord`) 구현
  ...
  - 중복 사용 체크
  - 끝말잇기 규칙 검증
- [x] **Step 9**: 한국어 두음법칙 처리 로직 구현
  - 'ㄹ' → 'ㅇ' 변환 (예: '리' → '이')
  - 'ㄴ' → 'ㅇ' 변환 (예: '뇨' → '요')
  - 받침 처리 (예: '잠만보' → '보' 또는 '뽀')
- [x] **Step 10**: 게임 상태 관리 (React Context 또는 상태 라이브러리)
  - 현재 턴, 사용된 단어 목록, 게임 상태 등
- [x] **Step 11**: 게임 종료 조건 구현 (패배/승리 판정)
- [x] **Step 12**: 힌트 생성 알고리즘 구현
  - 규칙에 맞는 미사용 단어 하나 추출
  - 힌트 버튼 UI 추가
...
- [x] **Step 36**: 포켓몬 상세 정보 페이지 구현
- [x] **Step 37**: 도감 데이터 연동 (PokeAPI or Local Data)
